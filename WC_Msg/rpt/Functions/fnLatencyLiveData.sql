...
-- =============================================
-- SELECT * FROM rpt.fnLatencyLiveData (30, 'tix_otp', 'ID', DEFAULT, 80)
-- SELECT * FROM rpt.fnLatencyLiveData (30, 'tix_otp', 'ID', 'Indosat_Local', 80)

CREATE FUNCTION [rpt].[fnLatencyLiveData] (
...
BEGIN 
	...
	BEGIN
		...
		SELECT n.TimeFrom [Time],
				...
				UpTo2s, Bet2And5s, Bet5And10s, Bet10And20s, Bet20And30s, GreaterThan30s
		FROM
			(SELECT DISTINCT TimeFrom, SubAccountId, ConnId, Country, OperatorId,
					...
					SUM(CASE WHEN Latency_sec <= 2 THEN 1 ELSE 0 END) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) UpTo2s,
					SUM(CASE WHEN Latency_sec > 2 AND Latency_sec <= 5 THEN 1 ELSE 0 END) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) Bet2And5s,
					SUM(CASE WHEN Latency_sec > 5 AND Latency_sec <= 10 THEN 1 ELSE 0 END) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) Bet5And10s,
					SUM(CASE WHEN Latency_sec > 10 AND Latency_sec <= 20 THEN 1 ELSE 0 END) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) Bet10And20s,
					SUM(CASE WHEN Latency_sec > 20 AND Latency_sec <= 30 THEN 1 ELSE 0 END) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) Bet20And30s,
					SUM(CASE WHEN Latency_sec > 30 THEN 1 ELSE 0 END) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) GreaterThan30s
			FROM
				...
				FROM
					(SELECT s.TimeFrom, s.SubAccountId, s.ConnId, s.Country, s.OperatorId,
							...
							Latency_sec=ROUND(DATEDIFF(ms, s.CreatedTime, ISNULL(dl4.EventTime, GETUTCDATE()))/CAST(1000 AS DECIMAL(10,2)), 3)
					...
					FROM @tmp s
						...
						LEFT JOIN sms.DlrLog dl4 WITH (NOLOCK, INDEX(IX_DlrLog_UMID_StatusId), FORCESEEK)
							ON s.UMID = dl4.UMID
								AND (dl4.StatusId = 40 OR dl4.StatusId = 41)
					) m
				) w
			) n

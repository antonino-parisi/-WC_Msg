...
-- =============================================
-- SELECT * FROM rpt.fnWavecellLatencyLiveData_v2 (30, 'tix_otp', 'ID', DEFAULT, 80)
-- SELECT * FROM rpt.fnWavecellLatencyLiveData_v2 (30, 'tix_otp', 'ID', 'Indosat_Local', 80)

CREATE FUNCTION [rpt].[fnWavecellLatencyLiveData_v2] (
...
(
	...
	Percentile_Wavecell_latency decimal(16,2),
	Avg_Supplier_latency decimal(16,2),
	Percentile_Supplier_latency decimal(16,2),
	Avg_latency decimal(16,2),
	Percentile_Latency decimal(16,2)
) 
AS  
BEGIN 
	...
	BEGIN
		...
		SELECT n.TimeFrom [Time],
				...
				CAST(Percentile_Wavecell_Latency AS DECIMAL(10,2)) [Percentile Wavecell Latency (s)],
				CAST(Average_Supplier_Latency AS DECIMAL(10,2)) [Average Supplier Latency (s)],
				CAST(Percentile_Supplier_Latency AS DECIMAL(10,2)) [Percentile Supplier Latency (s)],
				CAST(Average_Latency AS DECIMAL(10,2)) [Average Latency (s)],
				CAST(Percentile_Latency AS DECIMAL(10,2)) [Percentile Latency (s)]
		FROM
			(SELECT DISTINCT TimeFrom, SubAccountId, ConnId, Country, OperatorId,
					...
					PERCENTILE_CONT(CAST(@Percentile/CAST(100 AS decimal(5,2)) AS decimal(2,1))) WITHIN GROUP (ORDER BY WaveCell_Latency_sec)
							OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) AS Percentile_Wavecell_Latency,
					AVG(Supplier_Latency_sec) OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) AS Average_Supplier_Latency,
					PERCENTILE_CONT(CAST(@Percentile/CAST(100 AS decimal(5,2)) AS decimal(2,1))) WITHIN GROUP (ORDER BY Supplier_Latency_sec)
							OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) AS Percentile_Supplier_Latency,
					...
					PERCENTILE_CONT(CAST(@Percentile/CAST(100 AS decimal(5,2)) AS decimal(2,1))) WITHIN GROUP (ORDER BY Latency_sec)
							OVER (PARTITION BY TimeFrom, SubAccountId, ConnId, Country, OperatorId) Percentile_Latency
			FROM
				(SELECT TimeFrom, SubAccountId, ConnId, Country, OperatorId,-- StatusId,
						WaveCell_Latency_sec = IIF(WaveCell_Latency_sec < 0, 0, WaveCell_Latency_sec),
						Supplier_Latency_sec = IIF(Supplier_Latency_sec < 0, 0, Supplier_Latency_sec),
						Latency_sec = IIF(Latency_sec < 0, 0, Latency_sec)
				FROM
					(SELECT s.TimeFrom, s.SubAccountId, s.ConnId, s.Country, s.OperatorId, s.StatusId,
							WaveCell_Latency_sec=ROUND(DATEDIFF(ms, s.CreatedTime, ISNULL(dl3.EventTime, GETUTCDATE()))/CAST(1000 AS DECIMAL(6,1)),3),
							Supplier_Latency_sec=IIF(s.StatusId < 30, NULL, ROUND(DATEDIFF(ms, ISNULL(dl3.EventTime, GETUTCDATE()), ISNULL(dl4.EventTime, GETUTCDATE()))/CAST(1000 AS DECIMAL(6,1)),3)),
							Latency_sec=IIF(s.StatusId < 40, NULL, ROUND(DATEDIFF(ms, s.CreatedTime, ISNULL(dl4.EventTime, GETUTCDATE()))/CAST(1000 AS DECIMAL(10,2)), 3))
					FROM @tmp s

...
-- =============================================
-- EXEC cp.[SmsLog_CampaignDownload] @AccountUid = 'DCB63E13-4FDE-E711-8147-02D85F55FCE7', @CampaignId = 33466, @UserId = 'EFA95606-53E7-473E-BD72-FEBA6F5AE27B', @PageSize = 10000
CREATE PROCEDURE [cp].[SmsLog_CampaignDownload]
...
BEGIN
	...
	SELECT 
		...
		sl.PriceContractCurrency Currency,
		--(sl.SegmentsReceived * sl.Price) AS Cost,
		sl.SegmentsReceived * sl.PriceContractPerSms Cost,
		--st.StatusId,
		--st.Final,
	...
	FROM sms.SmsLog sl  WITH (INDEX (IX_SmsLog_SubAccount_CreatedTime), NOLOCK)
		...
		INNER JOIN (
			...
			FROM cp.CmCampaign c
				...
				INNER JOIN dbo.Account sa ON c.SubAccountUid = sa.SubAccountUid
	...
	WHERE 
		sl.CreatedTime BETWEEN DATEADD(MINUTE, -1, cb.ScheduledAt) AND DATEADD(MINUTE, 20, cb.ScheduledAt) 

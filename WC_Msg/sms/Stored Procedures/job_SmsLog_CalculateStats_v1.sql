...
CREATE PROCEDURE [sms].[job_SmsLog_CalculateStats_v1]
AS
BEGIN
	...
	BEGIN
		...
		SELECT
			dbo.fnTimeRountdown(MIN(sl.CreatedTime), @TimeIntervalInMinsSms) as TimeFrom,
			DATEADD(MINUTE, @TimeIntervalInMinsSms, dbo.fnTimeRountdown(MIN(sl.CreatedTime), @TimeIntervalInMinsSms)) as TimeTill,
			ca.AccountUid,
			sl.SubAccountUid, 
			sl.Country,
			sl.OperatorId,
			...
			--SUM(sl.SegmentsReceived * IIF(sl.CostContractCurrency IS NOT NULL, sl.CostContract, ISNULL(sl.CostEUR, sl.Cost))) as CostContract,
		...
		WHERE 
			(sl.CreatedTime >= @TimeframeStart AND sl.CreatedTime < @TimeframeEnd)
			/* TODO: rewrite filters to dynamic query */
			AND (@SubAccountUid IS NULL OR (@SubAccountUid IS NOT NULL AND a.SubAccountUid = @SubAccountUid))
		...
		GROUP BY 
			...
			sl.CostContractCurrency,
			sl.PriceContractCurrency,
			DATEDIFF(MINUTE, @ConstRefDate, sl.CreatedTime) / @TimeIntervalInMinsSms

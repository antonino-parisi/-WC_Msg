...
CREATE PROCEDURE [sms].[job_SmsLog_CalculateStats_v2]
AS
BEGIN
	...
	BEGIN
		...
		FROM
			(SELECT s.*, 
				...
				IIF(dlRead.UMID IS NOT NULL, 1, 0) AS Converted -- Status 50 (READ) found in dlrlog
			...
			FROM sms.SmsLog s WITH (NOLOCK, INDEX(IX_SmsLog_CreatedTime))
				LEFT JOIN sms.DlrLog dlRead WITH (NOLOCK) ON s.UMID = dlRead.UMID AND dlRead.StatusId = 50 /* READ */

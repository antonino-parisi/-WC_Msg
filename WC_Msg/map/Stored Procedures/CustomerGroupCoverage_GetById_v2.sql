...
CREATE PROCEDURE [map].[CustomerGroupCoverage_GetById_v2]
...
BEGIN
	...
	BEGIN
		...
		SELECT rg.RoutingGroupId, rg.RoutingGroupName,
			...
			rtc.TierEntryId, rtc.ConnUid, sc.ConnId, rtc.Weight, rtc.Active
		FROM rt.RoutingGroup rg WITH (NOLOCK) 
			...
			INNER JOIN rt.SupplierConn sc WITH (NOLOCK) ON rtc.ConnUid = sc.ConnUid

...
-- EXEC [dbo].[usp_GetReportByCarrierDetails] @StartDate = '2016-08-02', @EndDate = '2016-08-02', @RouteId = 'All', @AccountId = 'All', @SubAccountId = 'Silverstreet_HQ'
CREATE  PROCEDURE [dbo].[usp_GetReportByCarrierDetails]  
	@StartDate DATE,
	@EndDate DATE,
	...
	CREATE TABLE #TEMPTABLE_livetraffic
	(
	...
	BEGIN
	insert into #TEMPTABLE_livetraffic
				...
				SUM(Price) as TotalPrice,SUM(Cost) as TotalCost			   		   
							...
							WHERE --TrafficRecord.SubAccountId  not in(SELECT SubAccountId FROM Account WITH(NOLOCK) where AccountId in('Netval1','KameleanGroup'))AND
				...
				case when Status='TRASHED' THEN count(*) ELSE 0 end  as TRASHED,
				 case when Status='NO ROUTE AVAILABLE' THEN count(*)ELSE 0 end  as NOROUTEAVAILABLE,
				   ...
				   SUM(Price) as TotalPrice,SUM(Cost) as TotalCost					   			   
							...
							--WHERE @SubAccount_Cursor = TrafficRecord.SubAccountId 
							WHERE  --TrafficRecordArchive.SubAccountId  not in(SELECT SubAccountId FROM Account WITH(NOLOCK) where AccountId in('Netval1','KameleanGroup'))AND
							 DatetimeStamp between @StartDate AND @EndDate  and RouteIdUsed NOT IN('profyleNotification','profyleNotification_dev')						
							and MessageType<>'MOOut'
							GROUP By  Operator.Country , Operator.OperatorName, TrafficRecordArchive.OperatorId,Status,RouteIdUsed									
						
						SELECT  ISNULL(Country,'unknown') as Country, ISNULL(OperatorName,'unknown') as OperatorName,OperatorId,RouteIdUsed,
						SUM(TotalMessages)as TotalMessages ,SUM(DELIVEREDTODEVICE)as DELIVEREDTODEVICE,
						SUM(DELIVEREDTOCARRIER)as DELIVEREDTOCARRIER,SUM(REJECTEDBYCARRIER)as REJECTEDBYCARRIER,
						SUM(SENT)as SENT,SUM(RECEIVEDFORPROCESSING)as RECEIVEDFORPROCESSING,SUM(TRASHED)as TRASHED,
					...
					SUM(cast(RECEIVEDFORPROCESSING as decimal))+SUM(cast(REJECTEDBYDEVICE as decimal)))/(sum(cast(TotalMessages as decimal))-sum(cast(PARTOFAMESSAGE as decimal)))*100 as WavecellDeliveryRate
						FROM #TEMPTABLE_livetraffic 
						group by Country,OperatorName,OperatorId,RouteIdUsed		
						having sum(cast(TotalMessages as decimal))-sum(cast(PARTOFAMESSAGE as decimal)) > 0
				...
				case when Status='TRASHED' THEN count(*) ELSE 0 end  as TRASHED,
				 case when Status='NO ROUTE AVAILABLE' THEN count(*)ELSE 0 end  as NOROUTEAVAILABLE,
				   ...
				   SUM(Price) as TotalPrice	,SUM(Cost) as TotalCost				   			   
						...
						SELECT  ISNULL(Country,'unknown') as Country, ISNULL(OperatorName,'unknown') as OperatorName,RouteIdUsed,
						OperatorId,SUM(TotalMessages)as TotalMessages ,SUM(DELIVEREDTODEVICE)as DELIVEREDTODEVICE,
						SUM(DELIVEREDTOCARRIER)as DELIVEREDTOCARRIER,SUM(REJECTEDBYCARRIER)as REJECTEDBYCARRIER,
						SUM(SENT)as SENT,SUM(RECEIVEDFORPROCESSING)as RECEIVEDFORPROCESSING,SUM(TRASHED)as TRASHED,
						SUM(NOROUTEAVAILABLE)as NOROUTEAVAILABLE,SUM(REJECTEDBYDEVICE) as REJECTEDBYDEVICE,
						SUM(PARTOFAMESSAGE) as PARTOFAMESSAGE,sum(TotalPrice) as TotalPrice,sum(TotalCost) as TotalCost,
						...
						(SUM(cast(DELIVEREDTODEVICE as decimal))+SUM(cast(DELIVEREDTOCARRIER as decimal))+SUM(cast(SENT as decimal))	+				
					SUM(cast(RECEIVEDFORPROCESSING as decimal))+SUM(cast(REJECTEDBYDEVICE as decimal)))/(sum(cast(TotalMessages as decimal))-sum(cast(PARTOFAMESSAGE as decimal)))*100 as WavecellDeliveryRate
						FROM #TEMPTABLE_livetraffic group by Country,OperatorName,OperatorId,RouteIdUsed			
				...
				case when Status='TRASHED' THEN count(*) ELSE 0 end  as TRASHED,
				 case when Status='NO ROUTE AVAILABLE' THEN count(*)ELSE 0 end  as NOROUTEAVAILABLE,
				   ...
				   SUM(Price) as TotalPrice	,SUM(Cost) as TotalCost				   			   
						...
						SELECT  ISNULL(Country,'unknown') as Country, ISNULL(OperatorName,'unknown') as OperatorName,RouteIdUsed,
						OperatorId,SUM(TotalMessages)as TotalMessages ,SUM(DELIVEREDTODEVICE)as DELIVEREDTODEVICE,
						SUM(DELIVEREDTOCARRIER)as DELIVEREDTOCARRIER,SUM(REJECTEDBYCARRIER)as REJECTEDBYCARRIER,SUM(SENT)as SENT,
						SUM(RECEIVEDFORPROCESSING)as RECEIVEDFORPROCESSING,SUM(TRASHED)as TRASHED,SUM(NOROUTEAVAILABLE)as NOROUTEAVAILABLE,
						SUM(REJECTEDBYDEVICE) as REJECTEDBYDEVICE,SUM(PARTOFAMESSAGE) as PARTOFAMESSAGE,sum(TotalPrice) as TotalPrice,sum(TotalCost) as TotalCost,
						SUM(cast(DELIVEREDTODEVICE as decimal))/(sum(cast(TotalMessages as decimal))-sum(cast(PARTOFAMESSAGE as decimal)))*100 as DeliveryRate,
						(SUM(cast(DELIVEREDTODEVICE as decimal))+SUM(cast(DELIVEREDTOCARRIER as decimal))+SUM(cast(SENT as decimal))	+				
					SUM(cast(RECEIVEDFORPROCESSING as decimal))+SUM(cast(REJECTEDBYDEVICE as decimal)))/(sum(cast(TotalMessages as decimal))-sum(cast(PARTOFAMESSAGE as decimal)))*100 as WavecellDeliveryRate
						FROM #TEMPTABLE_livetraffic group by Country,OperatorName,OperatorId,RouteIdUsed															 
						HAVING (sum(cast(TotalMessages as decimal))-sum(cast(PARTOFAMESSAGE as decimal))) > 0										
						--drop table #TEMPTABLE_livetraffic
				...
				case when Status='TRASHED' THEN count(*) ELSE 0 end  as TRASHED,
				 case when Status='NO ROUTE AVAILABLE' THEN count(*)ELSE 0 end  as NOROUTEAVAILABLE,
				   ...
				   SUM(Price) as TotalPrice	,SUM(Cost) as TotalCost				   			   
						...
						SELECT  ISNULL(Country,'unknown') as Country, ISNULL(OperatorName,'unknown') as OperatorName,OperatorId,SUM(TotalMessages)as TotalMessages ,SUM(DELIVEREDTODEVICE)as DELIVEREDTODEVICE,SUM(DELIVEREDTOCARRIER)as DELIVEREDTOCARRIER,RouteIdUsed,
